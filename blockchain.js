...
class Blockchain {
  ...
  hashBlock(previousBlockHash, transactions) {
    return sha256(
      previousBlockHash + JSON.stringify(transactions), //we stringify it because hash functions only work on strings
    );
  }
  ...
  isValidNextScan(tvId, newLocation, newHandler) {
    ...
    //collects all transactions from all blocks and flattens them into a single list so smart contracts can check the full history then filters all transactions belonging to one tv only
    ...
    return (
      locationOrder.indexOf(newLocation) ===
